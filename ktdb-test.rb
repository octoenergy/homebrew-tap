...
class KtdbTest < Formula
  include Language::Python::Virtualenv

  desc "Database Tools for Kraken Tech"
  homepage "https://github.com/octoenergy/ktdb/"
  url "https://nexus.ktl.net/repository/pypi-kraken/packages/ktdb/1.1.0/ktdb-1.1.0.tar.gz",
      using: CustomCurlDownloadStrategy
  sha256 "f606cbc46802e3d60fd499e15405f2e4e05318ab1d145d230dbab2d86c3feed2"
  head "https://github.com/octoenergy/ktdb.git", branch: "main"
  ...
  depends_on "libpq" # brew link --force libpq
  depends_on "pgcli" => :recommended
  ...
  def install
    ...
    system venv.root / "bin/python3.13",
           "-m",
           "pip",
           "install",
           "--prefer-binary",
    ...
    cd buildpath do
      system venv.root / "bin/python3.13", "-m", "uv", "sync", "--no-dev"
    end

    # Install the main package from the tarball
    system venv.root / "bin/python3.13", "-m", "uv", "pip", "install", buildpath

    bin.install_symlink(venv.root / "bin/ktdb")
  end

  def post_install
#     # Generate shell completions
#     generate_completions_from_executable(bin / "ktdb", "completion")
  end

  test do
    assert_match "ktdb, version", shell_output("#{bin}/ktdb --version")
  end
end
